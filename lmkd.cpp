...
static int kill_one_process(struct proc* procp, int min_oom_score, int kill_reason,
    ...
    if (kill_desc) {
        ALOGI("Kill '%s' (%d), uid %d, oom_score_adj %d to free %ldkB; reason: %s", taskname, pid,
              uid, procp->oomadj, tasksize * page_k, kill_desc);
    } else {
        ALOGI("Kill '%s' (%d), uid %d, oom_score_adj %d to free %ldkB", taskname, pid,
...
static void update_props() {
    ...
    stall_limit_critical = property_get_int64("stall_limit_critical", 100);
}
